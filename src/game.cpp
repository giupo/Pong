...
void Game::init() {
  ...
  UInt32 subsystems = SDL_INIT_VIDEO | SDL_INIT_TIMER | SDL_INIT_AUDIO | SDL_INIT_EVENTS;
  if(SDL_Init(subsystems) & subsystems) {
    cout << "Cannot work it out, " << SDL_GetError() << endl;
    exit(1);
  }
  
  Locator::provide( new EventQueue() );
  Locator::provide( new ThreadPool() );
...
Game::~Game() {
  this->destroy();
  EventQueue* queue = Locator::getEventQueue();
  delete queue;
  SDL_Quit();
}
...
void Game::loop() {
  ...
  double delta = 0;
  while( running ) {
    EventQueue* eq = Locator::getEventQueue();
    eq->poll_events();
    if(!running) {
      return;
    }
